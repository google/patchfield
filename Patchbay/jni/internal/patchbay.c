...
static void perform_cleanup(patchbay *pb) {
  int i, j, k;
...
static int is_running(patchbay *pb) {
  return opensl_is_running(pb->os);
}

static int add_module(patchbay *pb,
  ...
  if ((pb->next_buffer + (input_channels + output_channels) *
    ...
    return -9;  // PatchbayException.OUT_OF_BUFFER_SPACE
  }
  int i;
  for (i = 0; i < MAX_MODULES; ++i) {
    audio_module *module = ami_get_audio_module(pb->shm_ptr, i);
  ...
  return -5;  // PatchbayException.TOO_MANY_MODULES
}

static int delete_module(patchbay *pb, int index) {
  audio_module *module = ami_get_audio_module(pb->shm_ptr, index);
  __sync_bool_compare_and_swap(&module->status, 1, 2);
  return 0;
}

static int activate_module(patchbay *pb, int index) {
  audio_module *module = ami_get_audio_module(pb->shm_ptr, index);
  __sync_bool_compare_and_swap(&module->active, 0, 1);
  return 0;
}
static int deactivate_module(patchbay *pb, int index) {
  audio_module *module = ami_get_audio_module(pb->shm_ptr, index);
...
static int get_output_channels(patchbay *pb, int index) {
  audio_module *module = ami_get_audio_module(pb->shm_ptr, index);
  return module->output_channels;
}

static int is_connected(patchbay *pb, int source_index, int source_port,
...
static int connect_modules(patchbay *pb, int source_index, int source_port,
   int sink_index, int sink_port) {
  if (!is_running(pb)) {
    perform_cleanup(pb);
  }
  ...
  return -7;  // PatchbayException.TOO_MANY_CONNECTIONS
}

static int disconnect_modules(patchbay *pb, int source_index, int source_port,
   int sink_index, int sink_port) {
  audio_module *sink = ami_get_audio_module(pb->shm_ptr, sink_index);
  int i;
  for (i = 0; i < MAX_CONNECTIONS; ++i) {
...
static patchbay *create_instance(int sample_rate, int buffer_frames,
    int input_channels, int output_channels) {
  patchbay *pb = malloc(sizeof(patchbay));
  if (pb) {
...
Java_com_noisepages_nettoyeur_patchbay_Patchbay_createInstance
...
Java_com_noisepages_nettoyeur_patchbay_Patchbay_releaseInstance
...
Java_com_noisepages_nettoyeur_patchbay_Patchbay_sendSharedMemoryFileDescriptor
(JNIEnv *env, jobject obj, jlong p) {
  patchbay *pb = (patchbay *) p;
  if (smi_send(pb->shm_fd) < 0) {
    LOGW("Failed to send file descriptor.");
    return -1;  // PatchbayException.FAILURE
  }
  return 0;
}

JNIEXPORT jint JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_start
(JNIEnv *env, jobject obj, jlong p) {
  patchbay *pb = (patchbay *) p;
...
JNIEXPORT void JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_stop
(JNIEnv *env, jobject obj, jlong p) {
  patchbay *pb = (patchbay *) p;
  opensl_pause(pb->os);
}
...
Java_com_noisepages_nettoyeur_patchbay_Patchbay_isRunning
(JNIEnv *env, jobject obj, jlong p) {
  patchbay *pb = (patchbay *) p;
...
JNIEXPORT jint JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_createModule
...
JNIEXPORT jint JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_deleteModule
...
JNIEXPORT jint JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_activateModule
(JNIEnv *env, jobject obj, jlong p, jint index) {
  patchbay *pb = (patchbay *) p;
...
JNIEXPORT jint JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_deactivateModule
(JNIEnv *env, jobject obj, jlong p, jint index) {
  patchbay *pb = (patchbay *) p;
...
JNIEXPORT jboolean JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_isActive
(JNIEnv *env, jobject obj, jlong p, jint index) {
  patchbay *pb = (patchbay *) p;
...
JNIEXPORT jboolean JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_isConnected
...
JNIEXPORT jint JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_getInputChannels
(JNIEnv *env, jobject obj, jlong p, jint index) {
  patchbay *pb = (patchbay *) p;
...
JNIEXPORT jint JNICALL
Java_com_noisepages_nettoyeur_patchbay_Patchbay_getOutputChannels
(JNIEnv *env, jobject obj, jlong p, jint index) {
  patchbay *pb = (patchbay *) p;
