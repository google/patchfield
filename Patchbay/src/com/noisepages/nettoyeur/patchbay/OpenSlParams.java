...
/**
 * This class illustrates how to query OpenSL config parameters on Jelly Bean
 * MR1 while maintaining backward compatibility with older versions of Android.
 * The trick is to place the new API calls in an inner class that will only be
 * loaded if we're running on JB MR1 or later.
 * 
 * This class is package private because the dependency of Patchbay on OpenSL is
 * an implementation detail that is not part of the public interface.
 */
abstract class OpenSlParams {
	...
	static OpenSlParams createInstance(Context context) {
		return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) ?
				new JellyBeanMr1OpenSlParams(context) : new DefaultOpenSlParams();
	}
